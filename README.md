...
Make sure you have the backend running, and restart the client. You should now be able to fetch
...
Next, let's add a rest call to the client: 
...
The client should now display the current time at the server: 

![Client with backend attached](client.png)
...
This results in a production build of the client in `client/build`: 
...
This will ensure that the client build files are copied after they have been generated by `npm build`.
